...
object Huffman {
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case left::right::xs => (makeCodeTree(left,right)::xs).sortWith((x,y) => weight(x) < weight(y))
      case _ => trees
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] =  {
      def decode2(tree2: CodeTree, bits: List[Bit]): List[Char] = tree2 match {
        ...
        case Fork(l, r, _, _) => if (bits.head == 1) decode2(r, bits.tail) else decode2(l, bits.tail)
      }
      decode2(tree,bits)
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      text.map(c => helper(tree)(c)).flatten
    }
