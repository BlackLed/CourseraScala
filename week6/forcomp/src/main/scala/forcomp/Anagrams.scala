...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(c => c.toLower).map(c => (c._1, c._2.length)).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    x.map(e => {
      if (y.exists(cy => cy._1 == e._1))
        (e._1, e._2 - y.find(l => e._1 == l._1).get._2)
      else e
    }).filter(c => c._2 != 0).sorted
    ...
    def helper(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        for {
          ...
          a <- helper(subtract(occurrences, wordOccurrences(w)))
          if !c.isEmpty
        } yield w::a
      }
    }
